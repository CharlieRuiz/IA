...
class ZergAgent(base_agent.BaseAgent):
  ...
  def my_attack(self, obs):
    ...
    if len(zerglings) >= 10 and len(hydras) >= 3 :
        #send attack at attack locations
        print("ataquee5555555")
        if self.unit_type_is_selected(obs, units.Zerg.Zergling) or self.unit_type_is_selected(obs, units.Zerg.Hydralisk):
            print("ataque00000000")
            if self.can_do(obs, actions.FUNCTIONS.Attack_minimap.id):
